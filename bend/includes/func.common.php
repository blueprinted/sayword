...
function template($filename, $fileext = TPL_FILEEXT) {
	return APP_ROOT."/templates/$filename.".$fileext;
}
 ...
 * @return void
 */
function apimessage($status, $message = '', $data = '') {
...
/**	功能 上传rar或zip文件至api
 *	@param $filename ssf文件名(全路径) 必须
 *	@param $fname 上传后的文件名(如：2014103015503012457895.ssf) 必须
 *	@param $dirname 上传保存的目录(如：upload/uploadFile/201410/30，相对于 API_ROOT/pc/data 这个目录) 必须
	@return Mixed(Array/Integer)
		Array : 上传完成(不一定成功，需要查看array['status'])
		0 : 文件不存在
		-1 : fopen打开文件失败
		...
		-4 : 返回数据格式不正确(json解码失败)
 */
function upload_rar2api($filename, $fname, $dirname){
	if(!file_exists($filename)){
		return 0;
	}
	if(!$fp = @fopen($filename, 'r')) {
	...
	$fields['fname'] = $fname;
	$fields['dirname'] = $dirname;
	$key = 'sapp_campus2015_save_exprar';//接口key(请求方与接收方一致)
	$fields['sign'] = md5('save_file'.$key);;
	$size = @filesize($filename);
	if(!$ch = @curl_init()) {
		return -2;
	}
	curl_setopt($ch, CURLOPT_URL, 'http://shouji.sogou.com/api/interface/interface_save_exprar.php');
	curl_setopt($ch, CURLOPT_POST, count($fields));
	curl_setopt($ch, CURLOPT_POSTFIELDS, $fields);
	curl_setopt($ch, CURLOPT_INFILE, $fp);
	curl_setopt($ch, CURLOPT_INFILESIZE, $size);
 ...
 *  @param
 * */
function sResource2https($url, $https_mode = false){
	if(HTTPS_MODE || $https_mode) {
		$url = str_replace("http://img.shouji.sogou.com/", "https://img-minput.sogoucdn.com/", $url);
		$url = str_replace("http://wap.dl.pinyin.sogou.com/", "https://wapdl-minput.sogoucdn.com/", $url);
		$url = str_replace("http://shouji.sogou.com/", "https://shouji.sogou.com/", $url);
		$url = preg_replace("/http:\/\/(\w+)\.sogoucdn\.com/i", "https://\\1.sogoucdn.com/", $url);
	}
	return $url;
}
?>
