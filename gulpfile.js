...
gulp.task('html',callback => {
    gulp.src(['rev/**/*.json', 'src/*.html', '!src/debug.html'])
...
gulp.task('fonts',function(callback){
    gulp.src(
        ...
        .pipe(gulp.dest('dist/static/c/fonts/'));
    callback();
});
...
gulp.task('image',function(callback){
    gulp.src('src/static/i/**/*',{base:'src'})//要处理的图片目录为img目录下的所有的.jpg .png .gif 格式的图片;
        ...
        .on('error', function (err) {
            gutil.log(gutil.colors.red('[ErrorImage]'), err.toString());
        })
        .pipe(gulp.dest('dist/'));
    callback();
});
...
gulp.task('default', gulp.series(['css', 'audio', 'fonts', 'js', 'jsd', 'image', 'html', 'copy'], function(callback){
    callback()
}));
